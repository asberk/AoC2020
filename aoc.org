...
** Day 08
   ...
   #+begin_src lisp :exports both :results output :tangle day08.lisp
     ...
     (defmacro with-check-visited (&body body)
     ...
     (defmacro with-shortcircuit (max-iters &body body)
       `(cond ((> (length *visited*) ,max-iters)
               ;; (format t "short-circuiting~%")
               (setf *running* nil))
              ((= *pointer* (length *instructions*))
               (format t "condition satisfied.~%accumulator is: ~a.~%" *accumulator*)
               (setf *running* nil)
               ,*accumulator*)
              (*running*
               (push *pointer* *visited*)
     ...
     (defun eval-instruction (inst)
       (when inst (with-check-visited (funcall (car inst) (cdr inst)))))
     ...
     (defun get-inst-symbol (pointer-val)
       "Gets the symbol associated with a pointer value"
