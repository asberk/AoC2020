...
** Day 19
   ...
   #+begin_src lisp :exports both :results output :tangle day19.lisp :shebang "#!/usr/bin/env sbcl --script"
     ;;;; see here: https://pastebin.com/raw/9N4u0gNi
     (load "my-utils.lisp")
     (load "~/.sbclrc")
     (ql:quickload "split-sequence")
     ...
     (defun day19-part2 ()
       (loop for line in (parse-input2) while line
          count (loop for possibility in (check 0 (list line))
                   if(= 0 (length possibility)) return t
                   finally (return nil))))
...
** Day 25
   :PROPERTIES:
   :ID:       C2190524-95B2-499F-A638-F186E925E1FD
   :END:


   #+begin_src lisp :exports both :results output :tangle day25.lisp :shebang "#!/usr/bin/env sbcl --script"
     (load "my-utils.lisp")
