...
** Day 20
   ...
   #+begin_src lisp :exports both :results output :tangle day20.lisp
     ...
     (defun unassigned-mutual-matches (tn1 tn2)
       (let* ((mutual-matches (find-mutual-matches tn1 tn2))
              (already-seen (assigned-tiles)))
         (set-difference mutual-matches already-seen)))

     (defun unassigned-matches (tile-name)
       (let* ((matches (get-tile-names-matching tile-name))
              (already-seen (assigned-tiles)))
     ...
     (defun arrange-tiles ()
       (let* ((corners (find-corners))
              ...
              (matches-first-corner (get-tile-names-matching first-corner)))
         ;; initialize
         ...
         ;; begin fill
         ;; (testing...)
         (unassigned-mutual-matches  (car matches-first-corner) (cadr matches-first-corner))
         (unassigned-matches (car matches-first-corner))))
     ...
     (defun without-corner (i)
       #'(lambda (corners)
           (loop for corner in corners for j from 0 unless (= j i) collect corner)))

     (defun arrange-edge (edge idx direction)
       (cond
         ((and (/= idx 0) (/= idx 11)) (error "arrange-edge: IDX must be 0 or 11"))
         ((eql direction :right)
          (loop for node in edge for j from 0
             do (setf (aref *arrangement* idx j) node)))
         ((eql direction :down)
          (loop for node in edge for i from 0
             do (setf (aref *arrangement* i idx) node)))))

     (defun initialize-arrangement ()
       "Uses a breadth-first search to find three of the edges of *ARRANGEMENT*."
       (arrange-edge (reverse (breadth-first-search #'cdr)) 0 :right)
       (arrange-edge (breadth-first-search (without-corner 2)) 11 :down)
       (arrange-edge (reverse (breadth-first-search (without-corner 3))) 11 :right))

     ;;; It would be cool if we could use VISITED returned from BREADTH-FIRST-SEARCH
     ;;; to triangulate the inner coordinates.
     ;;; Of course, it would probably be faster - computationally and IRL time-wise -
     ;;; to simply correctly orient the tiles from the get-go.


     (defun orient-tile (tile-name reference-tile)
       (when tile-name and reference-tile are neighbours and reference-tile is oriented
             do: orient tile-name
             ))
