...
** Day 24
   ...
   #+begin_src lisp :exports both :results output :tangle day24.lisp :shebang "#!/usr/bin/env sbcl --script"
     ...
     (defun get-angle (direction)
       (cond ((eql direction :east) 0)
             ((eql direction :west) pi)
             ((eql direction :north-east) (/ pi 3))
             ((eql direction :north-west) (* 2/3 pi))
             ((eql direction :south-west) (* -2/3 pi))
             ((eql direction :south-east) (* -1/3 pi))
             (t (error "direction not recognized: ~a" direction))))

     (defun get-vector (direction)
       (let ((angle (get-angle direction)))
     ...
     (defun round-to (number precision &optional (what #'round))
       "From here: http://www.codecodex.com/wiki/Round_a_number_to_a_specific_decimal_place#Common_Lisp"
       (let ((div (expt 10 precision)))
         (/ (funcall what (* number div)) div)))

     (defun cons-round (vector &optional (precision 4))
       (cons (round-to (car vector) precision) (round-to (cdr vector) precision)))

     (defun get-coords (directions)
       (apply #'cons+ (mapcar #'get-vector directions)))

     (defun get-rounded-coords (directions &optional (precision 4))
       (cons-round (get-coords directions) precision))

     (defun build-tile-flip-hash (&optional debug)
       (let ((tile-coords (mapcar #'get-rounded-coords (parse-input debug)))
             (h (make-hash-table :test 'equal)))
         (loop for coord in tile-coords
            do (if (null (gethash coord h))
                   (setf (gethash coord h) 1)
                   (incf (gethash coord h))))
         h))
     ...
     (defun get-neighbours (coord)
       (mapcar #'(lambda (direction) (cons-round (cons+ coord direction)))
               (mapcar #'get-vector (direction-names))))
     ...
     (defun num-black-neighbours (coord hash)
       (let* ((neighbours (get-neighbours coord))
              ...
              (num-black (count-if #'(lambda (val) (and val (oddp val))) flips)))
         num-black))
     ...
     (defun refresh-hash (hash)
       (loop for coord being the hash-keys of hash using (hash-value val)
          if (black-p coord hash)
          do (progn (setf (gethash coord hash) 1)
                    (let ((neighbours (get-neighbours coord)))
     ...
     ;;; refresh-hash creates problems due to the rounding error:
     (car (pairwise-distances (refresh-hash (build-tile-flip-hash :debug))))
     ;;; we need to find an alternative way. And it's probably going to come down to
     ;;; array indexing :-(
     ...
     (gethash (cons 0 0) (build-tile-flip-hash))
     (flip-black-p (cons 0 0) (build-tile-flip-hash))
     ...
     (defun update-tiles (hash)
       (let ((h (make-hash-table :test #'equal)))
         (loop for coord being the hash-keys of (refresh-hash hash)
            ...
            do (setf (gethash coord h) 1)
            else do (setf (gethash coord h) 0))
         h))

     (defun run-simulation (days &optional debug)
       (let ((hash (build-tile-flip-hash debug)))
         (when (= days 1)
           (return-from run-simulation (count-black-tiles hash)))
         (format t "Day 1: ~a~%" (count-black-tiles hash))
         (loop for i from 1 below days
            with black-tile-count = 0
            do (setf hash (update-tiles hash))
            do (setf black-tile-count (count-black-tiles hash))
            do (format t "Day ~a: ~a~%" (1+ i) black-tile-count)
            finally (return black-tile-count))))

     (run-simulation 10 :debug)
